#Tree_metadata <- read_csv("Tree_level_metadata.csv")
library(ggplot2)
...
desired_order <- c("Malus angustifolia", "Malus coronaria", "Malus ioensis","Malus fusca", 
                   ...
                   "Malus toringoides",
                   "Malus transitoria")
Tree_metadata$genotype <- factor(Tree_metadata$genotype, levels = desired_order)
...
#ggsave("PC3Figure6.pdf",plot=last_plot())
...
library(dplyr)
meancheck<- Tree_metadata %>%
  #group_by(genotype) %>%
  summarise(mean_length = mean(length, na.rm = TRUE))

varcheck<- Tree_metadata %>%
  group_by(genotype) %>%
