...
mdata = pd.read_csv('Malus_domestica_sieversii_metadata.csv') # read in csv
...
# READ IN CSV FROM CORRECT DIRECTORY
data_dir = "Malus_domestica_sieversii_JPEGs/" # set data directory # make sure there is a / at the end of the path
...
mdata = pd.read_csv("Malus_wild_metadata.csv") # read in csv
...
# READ IN CSV FROM CORRECT DIRECTORY
domesticated_data = pd.read_csv("Malus_domestica_sieversii_metadata.csv") # read in domesticated csv
...
theme_bw() +
  theme(
    ...
    panel.grid.minor = element_blank()   
      ...
      scale_x_continuous(breaks = pretty(Apple_metadata$PC1, n = 5)) +
  geom_smooth(method=lm, color="red", se=FALSE) +
   labs(
    x = "PC1",  
    y = "ln(width, cm)"   
...
correlation_data <- correlation_data %>% 
  rename("trait 1" = row, "trait 2" = col)
#write.csv(correlation_data, "tables2.csv")
...
ggplot(Apple_metadata, aes(x=PC1, y=aspect)) + 
  geom_point() +
theme_bw() +
  theme(
    panel.grid.major = element_blank(), 
...
ggplot(Apple_metadata, aes(x=PC3, y=solidity)) + 
  geom_point() +
theme_bw() +
  theme(
    panel.grid.major = element_blank(), 
...
#ggsave("Figure3b_c.pdf", plot=last_plot())
...
```{r}
#Tree_metadata<- read.csv("Tree_level_metadata.csv")
#####Figure 5: PC1/continent boxplot#####
library(ggthemes)
Tree_metadata %>% ggplot(aes(x = continent, y = width, fill = continent)) +
  geom_boxplot() +
  theme_few() +
...
#ggsave("Figure5a.pdf",plot=last_plot(), width=2, height=4, units="in")

#Mann-Whitney U test for PC1
wilcox.test(PC1~continent, data=Tree_metadata)
#Plot edits and merging with PC1 box plots were done in Affinity
```
...
Tree_metadata %>% ggplot(aes(x = continent, y = PC3, fill = continent)) +
  xlab("Tree level") +
  ylab("PC3 (11.2%)")+
  geom_boxplot() +
  theme_few() +
...
#ggsave("FigureS3.pdf",plot=last_plot(), width=8.0, height=9.0)

wilcox.test(PC3~continent, data= Tree_metadata)
```
...
pvals <- sapply(vars_to_test, function(var) {
  wilcox.test(Tree_metadata[[var]] ~ Tree_metadata$continent)$p.value
})
